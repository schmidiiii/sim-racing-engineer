...
function valueToColor(val: number, mode: MapChannel, lo: number, hi: number): string {
  ...
  if (mode === 'Throttle') {
    return `rgb(${Math.round((1 - t) * 80)},${Math.round(80 + t * 175)},${Math.round((1 - t) * 80)})`
  }
  if (mode === 'Brake') {
    return `rgb(${Math.round(80 + t * 175)},${Math.round((1 - t) * 80)},${Math.round((1 - t) * 80)})`
  }
...
function normalizePoints(
  ...
  const minLat = Math.min(...lats.slice(0, n)), maxLat = Math.max(...lats.slice(0, n))
  const minLon = Math.min(...lons.slice(0, n)), maxLon = Math.max(...lons.slice(0, n))
...
// Threshold + local-minima detection. 93% catches fast sweepers without false positives on straights.
function detectCornerDists(speedKmh: number[], lapDistPct: number[]): number[] {
...
export default function TrackMap() {
    ...
    const fetchAll = async () => {
      ...
      for (const key of selectedLapKeys) {
        ...
        try {
          ...
          const latData = results[0][0], lonData = results[1][0], valData = results[2][0]
          ...
          if (latRange > bestRange) {
            ...
            const result = normalizePoints(
              latData.samples, lonData.samples, transformed, latData.timestamps, latData.lap_dist_pct
            )
            bestPoints = result.points
            if (speedData) {
              bestCornerDists = detectCornerDists(speedData.samples.map(v => v * 3.6), speedData.lap_dist_pct)
            }
          }
        } catch { continue }
      }
  ...
  // Wheel zoom — attached directly so preventDefault works (non-passive)
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    setVb(cur => {
      ...
      return { x: mx - (mx - cur.x) * (newW / cur.w), y: my - (my - cur.y) * (newH / cur.h), w: newW, h: newH }
    })
  }, [])

  // Always attach listener — svgRef is always mounted (SVG rendered unconditionally)
  useEffect(() => {
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    dragRef.current = { x: e.clientX, y: e.clientY, vx: vbRef.current.x, vy: vbRef.current.y }
  }, [])
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const dx = (e.clientX - dragRef.current.x) / rect.width * cur.w
    const dy = (e.clientY - dragRef.current.y) / rect.height * cur.h
    setVb(v => ({ ...v, x: dragRef.current!.vx - dx, y: dragRef.current!.vy - dy }))
  }, [])
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Map — SVG always rendered so svgRef is valid for the wheel listener */}
      <div className="flex-1 min-h-0 flex items-center justify-center relative">
        {loading && <p className="absolute text-xs text-muted-foreground z-10">{t('loadingMap')}</p>}
        {showEmpty && <p className="absolute text-xs text-muted-foreground">{t('noGpsData')}</p>}
        <svg
          ref={svgRef}
          viewBox={`${vb.x} ${vb.y} ${vb.w} ${vb.h}`}
          preserveAspectRatio="xMidYMid meet"
          className="w-full h-full"
          style={{ maxHeight: '100%', cursor: 'grab', visibility: points.length > 0 ? 'visible' : 'hidden' }}
