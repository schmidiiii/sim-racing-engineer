...
function detectCorners(speedKmh: number[], lapDist: number[]): Corner[] {
  ...
  for (const c of candidates) {
    ...
    if (idx >= 0) { if (c.minSpeed < corners[idx].minSpeed) corners[idx] = c }
    else corners.push(c)
  }
  return corners.sort((a, b) => a.dist - b.dist)
}

function buildChartData(allLaps: LapCorners[]): { label: string; dist: number; [key: string]: number | string }[] {
  ...
  return ref.corners.map((refCorner, idx) => {
    const row: { label: string; dist: number; [key: string]: number | string } = {
      label: `T${idx + 1}`, dist: refCorner.dist,
    }
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      for (let ci = 0; ci < selectedLapKeys.length; ci++) {
        ...
        if (!sess || !sess.available_channels.find(c => c.name === 'Speed')) continue
        try {
          ...
          const corners = detectCorners(speedData.samples.map(v => v * 3.6), speedData.lap_dist_pct)
        ...
        <div style={{ height: Math.max(240, chartData.length * 14 + 60) }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 4, right: 12, left: 0, bottom: 4 }} barCategoryGap="20%">
              ...
              <YAxis domain={[yMin, yMax]} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} width={38} tickFormatter={v => `${v}`} />
              ...
              {lapCorners.map(ld => (
                <Bar key={ld.lapKey} dataKey={ld.lapKey} name={`L${ld.lapNumber}`} fill={getLapColor(ld.colorIndex)} radius={[3, 3, 0, 0]} maxBarSize={32}>
                  {chartData.map((_, i) => <Cell key={i} fill={getLapColor(ld.colorIndex)} fillOpacity={0.85} />)}
                </Bar>
              ))}
            </BarChart>
          </ResponsiveContainer>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                ...
                {lapCorners.length >= 2 && <th className="text-right pb-1.5 text-muted-foreground font-medium">Δ km/h</th>}
              </tr>
            </thead>
            <tbody>
              {chartData.map((row, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-border/40 last:border-0">
                    ...
                    {lapCorners.map(ld => (
                      <td key={ld.lapKey} className="py-1.5 pr-3 text-right font-mono tabular-nums" style={{ color: getLapColor(ld.colorIndex) }}>
